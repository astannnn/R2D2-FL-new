...
Client reliability scores \( r_k \) are computed using a server-side proxy dataset.
...
- Class-level reliability \( r_{k,c} \) is computed  
...
This mechanism reduces the influence of unreliable clients and improves robustness under label noise.
